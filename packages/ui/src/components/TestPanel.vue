  ...
  <ContentCardUI>
    <div class="flex flex-col h-full">
      ...
      <div class="flex items-center justify-between mb-4">
        <h3 class="text-lg font-medium">{{ t('test.panel') }}</h3>
        <button
          @click="isTestPanelVisible = !isTestPanelVisible"
          class="theme-button-secondary px-3 py-1 text-sm"
        >
          {{ isTestPanelVisible ? t('test.hidePanel') : t('test.showPanel') }}
        </button>
      </div>
      ...
      <div
        ...
        :style="{
          height: isTestPanelVisible ? 'auto' : '0',
          opacity: isTestPanelVisible ? 1 : 0,
          pointerEvents: isTestPanelVisible ? 'auto' : 'none'
        }"
      >
        <!-- Test Input Area -->
        <div class="flex-none">
