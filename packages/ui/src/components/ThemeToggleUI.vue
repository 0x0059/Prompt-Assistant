<template>
  <div class="relative">
    <button
      ...
      :aria-label="$t('theme.toggleThemeMenu')"
    >
      ...
      <span class="sr-only">{{ $t('theme.currentTheme', { name: getThemeDisplayName }) }}</span>
    </button>
    ...
    <div v-if="showThemeMenu" class="absolute right-0 mt-2 min-w-[132px] theme-dropdown z-50">
      <div class="py-1">
        <button 
          ...
          :aria-label="$t('theme.selectTheme', { name: theme.name })"
        >
...
// 定义主题类型
type ThemeId = 'light' | 'dark' | 'blue' | 'green' | 'purple';
...
const availableThemes = computed<Theme[]>(() => [
  ...
  {
    id: 'blue',
    name: i18n.t('theme.blue'),
    iconClass: 'text-blue-500',
    cssClass: 'theme-blue'
  },
  {
    id: 'green',
    name: i18n.t('theme.green'),
    iconClass: 'text-green-500',
    cssClass: 'theme-green'
  },
  {
    id: 'purple',
    name: i18n.t('theme.purple'),
...
const getThemeDisplayName = computed(() => {
  const theme = availableThemes.value.find(t => t.id === currentTheme.value);
  return theme ? theme.name : i18n.t('theme.title');
});

// Close dropdown menu when clicking outside
const handleClickOutside = (event: MouseEvent) => {
  if (showThemeMenu.value && event.target && !(event.target as Element).closest('.relative')) {
    showThemeMenu.value = false;
  }
};
...
onMounted(() => {
  ...
  requestAnimationFrame(() => {
    ...
    // Set current theme
    if (themeId && availableThemes.value.find(t => t.id === themeId as ThemeId)) {
